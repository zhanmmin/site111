(() => {
  ...
  function renderOverview() {
    ...
    const activities = (metrics.activity || []).map((item) => `<div><span class="admin-activity-icon"><i class="ph ph-database"></i></span><p>${escapeHtml(item.action)} · ${escapeHtml(item.resource_type)} ${escapeHtml(item.resource_id || "")}</p><time>${escapeHtml(item.created_at || "刚刚")}</time></div>`).join("") || '<div class="admin-empty"><i class="ph ph-clock"></i><strong>暂无操作日志</strong><span>管理员操作会在这里显示。</span></div>';
